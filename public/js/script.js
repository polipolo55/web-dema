...
class DemaOS {
  ...
  async loadWindowConfig() {
    try {
      ...
      if (
        Array.isArray(config?.startupWindows) &&
        config.startupWindows.length > 0
      ) {
        this.windowConfig = {
          startupWindows: config.startupWindows,
          windowLayouts:
            config.windowLayouts && typeof config.windowLayouts === "object"
      ...
      console.warn(
        "Could not load window configuration. Using defaults.",
        error,
      );
    }
  }
  ...
  _showAndPositionWindow(windowElement, windowId) {
    ...
    if (!this.windows.has(windowId)) {
      if (
        configuredLayout &&
        Number.isFinite(configuredLayout.x) &&
        Number.isFinite(configuredLayout.y)
      ) {
  ...
  _isGalleryDataReady() {
    return (
      ...
      this.galleryData.photos.length > 0
    );
  }
  ...
  _emitGalleryReady() {
    ...
    document.dispatchEvent(
      new CustomEvent("dema:gallery-ready", {
        detail: {
  ...
  initializeGridSystem() {
    ...
    this.gridSize =
      parseInt(computedStyle.getPropertyValue("--icon-spacing")) || 100;
    this.iconWidth =
      parseInt(computedStyle.getPropertyValue("--icon-width")) || 90;
    this.iconSize =
      parseInt(computedStyle.getPropertyValue("--icon-size")) || 56;
  ...
  positionIconsOnGrid() {
    ...
    const maxRows = Math.max(
      1,
      Math.floor((window.innerHeight - 100) / this.gridSize),
    ); // Account for taskbar
    const maxCols = Math.max(
      1,
      Math.floor((window.innerWidth - 16) / this.gridSize),
    );

    // Split icons into socials (links) and others
    const socialIcons = allIcons.filter((i) =>
      i.classList.contains("social-icon"),
    );
    const otherIcons = allIcons.filter(
      (i) => !i.classList.contains("social-icon"),
    );
    ...
    const neededSocialCols = Math.max(
      1,
      Math.ceil(socialIcons.length / maxRows),
    );
    // Start social columns at least after the last used column, but try to keep them to the far right
    const socialStartCol = Math.max(
      maxColUsed + 1,
      Math.max(0, maxCols - neededSocialCols),
    );
  ...
  updateTaskbarActive() {
    document
      ...
      .forEach((tw) => tw.classList.remove("active"));
  ...
  toggleWallpaper() {
    ...
    console.log(
      ...
      this.wallpaperIndex,
      this.wallpapers[this.wallpaperIndex],
    );
    ...
    try {
      ...
      if (usersListElement) {
        usersListElement.innerHTML =
          '<div style="padding: 20px; text-align: center; color: #666;">Error carregant usuaris</div>';
      }
    }
  }
...
window.addEventListener("resize", () => {
  ...
  document.querySelectorAll(".window").forEach((window) => {
    ...
    if (rect.left > window.innerWidth - 100) {
      window.style.left =
        Math.max(0, window.innerWidth - window.offsetWidth) + "px";
    }
    if (rect.top > window.innerHeight - 100) {
      window.style.top =
        Math.max(0, window.innerHeight - window.offsetHeight - 40) + "px";
    }
  });
});
  ...
  setTimeout(() => {
    if (
      ...
      const targetDate = new Date(
        this.countdownData.release.releaseDate,
      ).getTime();
...
DemaOS.prototype.showCountdownCompleted = function () {
  ...
  if (countdownDisplay) {
    countdownDisplay.style.background =
      "linear-gradient(45deg, #006600, #009900)";
  ...
  countdownNumbers.forEach((number) => {
    number.style.background =
      "linear-gradient(180deg, #00ff00 0%, #006600 100%)";
...
DemaOS.prototype.loadGalleryData = async function () {
  try {
    ...
    this.galleryData.photos = Array.isArray(this.galleryData.photos)
      ? this.galleryData.photos.map((photo, index) =>
          this.normalizeGalleryItem(photo, index),
        )
      : [];
...
DemaOS.prototype.normalizeGalleryItem = function (photo, index = 0) {
  ...
  const inferredByMime =
    normalized.mimeType && normalized.mimeType.startsWith("video")
      ? "video"
      : null;
  const inferredByFilename = this._guessMimeTypeFromFilename(
    normalized.filename || "",
  )?.startsWith("video/")
    ? "video"
    : null;
  ...
  normalized.mediaType =
    explicitType === "video" ||
    inferredByMime === "video" ||
    inferredByFilename === "video"
      ? "video"
      : "photo";
...
DemaOS.prototype.setGalleryLoadingState = function (isLoading, reason = "") {
  ...
  const activeMedia = document.querySelector(
    "#galleryWindow .main-media.is-active",
  );
  if (activeMedia) {
  ...
  document.dispatchEvent(
    new CustomEvent("dema:gallery-loading", {
      detail: {
...
DemaOS.prototype.initializeGallery = function () {
  if (
    ...
    if (galleryContent) {
      galleryContent.innerHTML =
        '<p style="text-align: center; padding: 20px;">No hi ha elements disponibles</p>';
    }
    const photoCounter = document.getElementById("photoCounter");
    if (photoCounter) {
  ...
  const sortedPhotos = [...this.galleryData.photos].sort(
    (a, b) => (a.order || 0) - (b.order || 0),
  );
  this.galleryData.photos = sortedPhotos.map((photo, index) =>
    this.normalizeGalleryItem(photo, index),
  );
...
DemaOS.prototype.renderThumbnails = function () {
  ...
  this.galleryData.photos.forEach((photo, index) => {
    ...
    button.setAttribute(
      "aria-label",
      photo.title ? `Obrir ${photo.title}` : `Element ${index + 1}`,
    );
    ...
    if (photo.mediaType === "video" && photo.thumbnail) {
      ...
      (photo.thumbnail || photo.filename)
    ) {
      const img = document.createElement("img");
...
DemaOS.prototype.displayPhoto = function (index) {
  if (
    !this.galleryData ||
    !Array.isArray(this.galleryData.photos) ||
    index < 0 ||
    index >= this.galleryData.photos.length
  ) {
  ...
  document.dispatchEvent(
    new CustomEvent("dema:gallery-media-change", {
  ...
  const handleReady = (type) => {
    ...
    document.dispatchEvent(
      new CustomEvent("dema:gallery-media-ready", {
  ...
  const handleError = (type) => {
    ...
    document.dispatchEvent(
      new CustomEvent("dema:gallery-media-error", {
        detail: { index, media: mediaItem, type },
      }),
    );
  };
  ...
  if (mediaItem.mediaType === "video") {
    if (videoEl) {
      if (videoSourceEl) {
        ...
        const mimeType =
          mediaItem.mimeType ||
          this._guessMimeTypeFromFilename(mediaItem.filename);
        if (mimeType) {
